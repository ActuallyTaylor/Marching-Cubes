...
public class Noise : MonoBehaviour
{
    ...
    public static float[,,] Generate3DNoiseMap(int chunkSize, int seed, float scale, int octaves, float persistance, float lacunarity, Vector3 offset)
    {
        ...
        for (int y = 0; y < chunkSize; y++)
        {
            for (int x = 0; x < chunkSize; x++)
            {
                for (int z = 0; z < chunkSize; z++)
                {
                    ...
                    else if (noiseDensity < minLocalNoiseDensity)
                    {
                        minLocalNoiseDensity = noiseDensity;
                    }
        ...
        for (int y = 0; y < chunkSize; y++)
        {
            for (int x = 0; x < chunkSize; x++)
            {
                for (int z = 0; z < chunkSize; z++)
                {
                    noiseMap[x, y, z] = Mathf.InverseLerp(minLocalNoiseDensity, maxLocalNoiseDensity, noiseMap[x, y, z]);
                }
            }
        }
